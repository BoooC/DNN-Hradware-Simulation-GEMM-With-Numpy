...
def read_matrices_from_file(file_path, matrix_size, channels):
    with open(file_path, 'r') as file:
        ...
        if len(data) % (matrix_size * matrix_size * channels) != 0:
            raise ValueError("The data size is not matched the matrix size")
...
def read_and_group_data(file_path, elements_per_group, num_groups):
    with open(file_path, 'r') as file:
        ...
        if len(data) != elements_per_group * num_groups:
            raise ValueError("The number of data is not matched group size")
...
def reshape_to_squares(arr, side_length):
    ...
    if rows != side_length * side_length:
        raise ValueError("size is not matched")
...
def merge_arrays(*arrays):
    if not arrays:
        raise ValueError("at least one array to be merged")
    
    shape = arrays[0].shape
    if not all(arr.shape == shape for arr in arrays):
        raise ValueError("all array size must be the same")
...
def reshape_array(arr, rows, cols):
    if arr.size != rows * cols:
        raise ValueError("array size must equals to rows * cols")
...
conv1_weight_path = 'weight/kernel1.txt' 
conv2_weight_path = 'weight/kernel2.txt' 
fc1_weight_path   = 'weight/kernel3.txt' 
fc2_weight_path   = 'weight/kernel4.txt' 
fc3_weight_path   = 'weight/kernel5.txt' 

ifmap_path = 'ifmap/5.txt' 
